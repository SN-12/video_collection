...
#profile = ""
profile1 = "/home/listic/Documents/video_collection/tools/res/profile1"
profile2 = "/home/listic/Documents/video_collection/tools/res/profile2"


#path to selenium chromedriver
driver = "/home/listic/Documents/video_collection/tools/res/chromedriver/chromedriver"
...
def collect_video(folder, URL1, URL2, length=1, netem=None, runTA=True, runServer=True, runPcap=True, ):
  #start tcpdump
  if runPcap:
    pcap = Command()
    netif = "wlo1"
    pcap.setCmd("sudo tcpdump -i {} -w {}dump.pcap".format(netif, folder))
    pcap.runAsync()

  #Start video stream
  streamer = Streamer()
  streamer.config(length=length, profile=profile1, driverPath=driver, URL=URL1)
  streamer.runAsync()

  # Start Second video stream
  streamer2 = Streamer()
  streamer2.config(length=length, profile=profile2, driverPath=driver, URL=URL2)
  streamer2.runAsync()
  ...
  pcap = Command()
  pcap.setCmd("sudo killall -2 tcpdump")
  pcap.runSync()
...
def collect_all_videos(folder): 
  ...
  length = 60
  #Netflix
  URL1 = 'https://www.netflix.com/watch/81102542?trackId=14170036&tctx=1%2C0%2Ce9e128ed-4677-410f-8dab-6878e151dc46-21841544%2Cceb20198-b2a5-4621-b0f3-3fa6a6879d50_35863833X19XX1623241546631%2C%2C'
  URL2 = 'https://www.netflix.com/watch/70298735'

  #Youtube
  #URL1 = 'https://www.youtube.com/watch?v=WSkyoyyvnAY&list=RDWSkyoyyvnAY&start_radio=1'
  #URL2 = 'https://www.youtube.com/watch?v=YCl-0lu0vHM'


  collect_video(folder, URL1, URL2, length, netem)
  httpd.force_stop()
...
def main():
  ...
  if args["open"] > 0:
    ...
    start_server_only(args["folder"])
      ...
      os.makedirs(args["folder"] + "/" + "experiment")
      collect_all_videos(args["folder"]+"/experiment"+'/')
